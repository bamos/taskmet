...
class OptimiseSubmodular(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        dZdYhat = OptimiseSubmodular._get_dZdYhat(
            Z.to(device=device), Yhat.to(device=device), get_obj, budget
        )
    ...
    def _get_dZdYhat(
        ...
        with torch.enable_grad():
            # Objective
            f = get_obj(Yhat_local, Z_local).to(device=device)
            # Gradient
            dfdZ = torch.autograd.grad(f, Z_local, create_graph=True)[0]
            # Hessian
            dfdZ_dZ = OptimiseSubmodular._get_elementwise_derivative(dfdZ, Z_local).to(
                device=device
            )
            # Cross-Term
        ...
        if torch.logical_and(Z > EPS, Z < 1 - EPS).any():
            lambda_sum = torch.mean(dfdZ[torch.logical_and(Z > EPS, Z < 1 - EPS)]).to(
                device=device
            )
        else:
            lambda_sum = torch.tensor(0).float().to(device=device)
        ...
        lambda_lower = torch.where(Z < EPS, dfdZ - lambda_sum, torch.zeros_like(Z)).to(
            device=device
        )
        ...
        diag_g = torch.diag(g).to(device=device)
